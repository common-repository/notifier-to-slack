 ...
 * Description: Notifier to Slack is the ultimate activity guard plugin, support alert, and security plugin for WordPress. It logs all WordPress actions and user activities, enhances website security, and instantly notifies you of support requests and reviews from WordPress.org. With real-time alerts sent directly to your Slack workspace, you can effortlessly track reviews, monitor support tickets, and stay on top of your site's activity for seamless management and security
 * Version:           3.1.1 
...
define( 'WP_NOTIFIER_TO_SLACK_VERSION', '3.1.1' );
...
use WPNTS\Inc\Postnotice;
use WPNTS\Inc\Activate;
use WPNTS\Inc\DbTables;
use WPNTS\Inc\Wpwidgets;
use WPNTS\Inc\Comment;
...
if ( ! class_exists('Notifier') ) {
	...
	class Notifier {
		...
		public function wpnts_plugin_activation() {
			...
			// Check if activation occurred within the last 5 days
			$activation_time = get_option( 'NotifierActivationTime' );
			// If we have an activation time, calculate the days since activation
			if ( $activation_time ) {
				$days_since_activation = floor( ( time() - $activation_time ) / ( 60 * 60 * 24 ) );
		
				// Check if setup wizard was completed
				$wizard_completed = get_option( 'notifier_ran_setup_wizard', false );
		
				// If user activated the plugin within the last 5 days and hasn't completed the wizard, ensure it's incomplete.
				if ( $days_since_activation <= 2 && !$wizard_completed ) {
					update_option( 'notifier_ran_setup_wizard', false ); 
				} elseif ( $days_since_activation > 2 ) {
					update_option( 'notifier_ran_setup_wizard', true );
				}
			}else{
				update_option( 'notifier_ran_setup_wizard', false );
			}
