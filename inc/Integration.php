...
class Integration {
	...
	public function wpnts_cf7_integration($contact_form) {
		...
		if ($submission) {
			...
			$log_message = "Contact Form 7 ID: $contact_forms_id submitted at $current_time";
			$severity = "Normal";
			...
			$attachmentHandler->log_activity_to_db( $severity, $time, $ip, $display_name='CF7', $event_type, $log_message  );
	...
	public function form_submit_action ( $contact ){
		...
		$log_message = "Formflow Form (ID: {$contact['form_id']}) submitted at $current_time";
		$severity = "Normal";
		$event_type = "Submission";
		$time = current_time( 'mysql' );
		...
		$attachmentHandler = new SlackAttachment();
		$ip = $attachmentHandler->get_user_ip();
		$attachmentHandler->log_activity_to_db( $severity, $time, $ip, $display_name='Formflow', $event_type, $log_message  );
