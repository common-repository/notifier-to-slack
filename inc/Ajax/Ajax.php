...
class Ajax {
	...
	public function __construct() {
		...
		add_action('wp_ajax_get_active_logged_in_users', [$this, 'get_active_logged_in_users']);

		add_action( 'wp_ajax_notice_settings', [ $this, 'notice_settings' ] );
		add_action( 'wp_ajax_activity_table', [ $this, 'activity_table' ] );
		...
		add_action( 'wp_ajax_wpnts_activity_data_resets', [ $this, 'activity_data_resets' ] );
	...
	public function activity_table() {
		...
		$wpnts_db_instance = new DB();
		$activity_logs = $wpnts_db_instance->get_all_activity_logs();
   ...
   public function install_and_activate_formflow_plugin() {
	...
	/**
	 * Truncate the activity log
	 */
	public function activity_data_resets() {
		if ( ! isset($_POST['nonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'wp_rest') ) {
			wp_send_json_error([
				'message' => __('Invalid nonce.', 'wpnts'),
			]);
		...
		$confirm = ! empty( $_POST['confirm'] ) ? filter_var( $_POST['confirm'], FILTER_VALIDATE_BOOLEAN ) : false;

		if ( $confirm === true ) {
			$wpnts_db_instance = new DB();
			$response = $wpnts_db_instance->activity_log_truncate( $confirm );

			if ( $response ) {
				wp_send_json_success([// phpcs:ignore
					'message' => __('Truncate success.', 'wpnts'),
					'status' => __('success', 'wpnts'),
				]);
			}

			wp_send_json_error([
				'message' => __('Truncate failed.', 'wpnts'),
				'status' => __('failed', 'wpnts'),
			]);
		}

		wp_send_json_error([
			'message' => __('Truncate errors.', 'wpnts'),
			'status' => __('errors', 'wpnts'),
		]);
	}
	 ...
	 * Get active users data from options table
	 */
	public function get_active_logged_in_users() {
        // Check nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'wp_rest')) {
            wp_send_json_error([
                'message' => __('Invalid nonce.', 'wpnts'),
		...
		// If active_users_data is empty, populate it with details of the current active user.
		if (empty($active_users_data)) {
			...
			// Update the active users option
			update_option('active_users_data', $active_users_data);
		}
        ...
        $total_active_users = count($active_users_data);

        // Extract user names and login times
        $user_names = [];
        $login_time = [];
        foreach ($active_users_data as $user_id => $user_data) {
            $user_names[$user_id] = $user_data['user_name'];
            $login_time[$user_id] = $user_data['activation_time'];
        }
