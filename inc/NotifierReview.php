...
class NotifierReview {
	...
	public function __construct() {
		...
		add_action('wpnts_corn_hook', [ $this, 'wpnts_reset_activity_log_on_duration' ]);
	}
	...
	/**
	 * Function to check logdurationtime and truncate the table.
	 *
	 * @since 1.0.0
	 */
	public function wpnts_reset_activity_log_on_duration() {
		
		global $wpdb;
	
		// Fetch the global API settings from the options table.
		$wpnts_global_api_settings = get_option('wpnts_global_api_settings');
		if ($wpnts_global_api_settings) {
			$settings = json_decode($wpnts_global_api_settings, true);
	
			// Get logdurationtime and the current date.
			$logdurationtime = isset($settings['logdurationtime']) ? $settings['logdurationtime'] : '';
			$current_date = current_time('Y-m-d');
	
			// Check if the current date matches logdurationtime.
			if ($logdurationtime && $current_date >= $logdurationtime) {
	
				// Check if the function has already run today.
				$last_run_date = get_option('wpnts_last_reset_activity_log_date', false);
				if ($last_run_date === $current_date) {
					// Function has already run today, so do not execute the truncation.
					return;
				}
				
				// Truncate the activity log table.
				$activity_log_table = $wpdb->prefix . 'wpnts_activity_log';
				$wpdb->query("TRUNCATE TABLE $activity_log_table");
	
				// Optionally, reset the logdurationtime to the next interval.
				$activitylogduration = isset($settings['activitylogduration']) ? intval($settings['activitylogduration']) : 30;
				
				// Calculate the new logdurationtime (today + activitylogduration days).
				$next_reset_date = date('Y-m-d', strtotime("+$activitylogduration days", strtotime($current_date)));
	
				// Update the logdurationtime to the next reset date.
				$settings['logdurationtime'] = $next_reset_date;
				update_option('wpnts_global_api_settings', json_encode($settings));
	
				// Update the last run date to today's date.
				update_option('wpnts_last_reset_activity_log_date', $current_date);
			}
		}
	}
