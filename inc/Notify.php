...
class Notify {
	...
	public function __construct() {
		...
		$pluginupdate = $schedules_interval->pluginupdate ?? 'false';
		...
		if ( true === $pluginupdate ) {
			add_action('upgrader_process_complete', [$this, 'wpnts_plugin_updated'], 10, 2);
		}
	...
	public function wpnts_plugin_activation( $plugin, $network_wide ) {
		 ...
		 * Plugin activity log
		 */
		$plugin_name = strstr($plugin, '/', true);
		$log_message = "Plugin <b>'$plugin_name'</b> has been activated by 👉 <b>$current_user_name.</b>";
		$severity = "High";
    	...
    	$attachmentHandler->log_activity_to_db( $severity, $time, $ip, $current_user_name, $event_type, $log_message  );
	...
	public function wpnts_plugin_deactivated( $plugin, $network_deactivating ) {
		...
		/**
		 * Save to DB for activity log
		 */
		$plugin_name = strstr($plugin, '/', true);
		$log_message = "Plugin <b>'$plugin_name'</b> has been deactivated by 👉<b> $current_user_name</b>";
		$severity = "High";
	...
	public function wpnts_user_activity_notification_login( $user_login, $user ) {
		...
		$log_message = "User 👉 <b> $user_login </b> has logged in.";
		$severity = "High";
		$event_type = "Login";
		$time = current_time( 'mysql' );
		...
		$attachmentHandler = new SlackAttachment();
		$ip = $attachmentHandler->get_user_ip();
    	$attachmentHandler->log_activity_to_db( $severity, $time, $ip, $user_login, $event_type, $log_message  );
	...
	public function wpnts_user_activity_notification_logout() {
		 ...
		 */
		 ...
		 $log_message = "User 👉 <b>$user_display_name</b> has logged out.";
		 $severity = "Normal";
		 $event_type = "Logout";
		 $time = current_time( 'mysql' );
		 ...
		 $attachmentHandler = new SlackAttachment();
		 $ip = $attachmentHandler->get_user_ip();
		 $attachmentHandler->log_activity_to_db( $severity, $time, $ip, $user_display_name, $event_type, $log_message  );
	...
	public function wpnts_new_user_notification( $user_id ) {
		 ...
		 */
		 ...
		 $attachmentHandler = new SlackAttachment();
		 $ip = $attachmentHandler->get_user_ip();
		 $attachmentHandler->log_activity_to_db( $severity, $time, $ip, $user->user_login, $event_type, $message  );
	 ...
	 * Plugin update
	 */
	public function wpnts_plugin_updated($upgrader_object, $options) {
		...
		if ($options['action'] == 'update' && $options['type'] == 'plugin') {
			...
			foreach ($options['plugins'] as $plugin) {
				...
				$this->wpnts_notify_slack("Plugin '{$plugin_name}' has been updated.", ":package:");
				
				
				$log_message = "Plugin <b>'$plugin_name'</b> has been updated by 👉 <b>$updated_by.</b>.";
				$severity = "High";
				$event_type = "Updated";
				$time = current_time( 'mysql' );
